...
static az_span COMMAND_NAME_FORWARD = AZ_SPAN_FROM_STR("F");
static az_span COMMAND_NAME_BACKWARD = AZ_SPAN_FROM_STR("B");
static az_span COMMAND_NAME_LEFT = AZ_SPAN_FROM_STR("L");
static az_span COMMAND_NAME_RIGHT = AZ_SPAN_FROM_STR("R");
static az_span COMMAND_NAME_STOP = AZ_SPAN_FROM_STR("S");
...
static bool car_wheel_up = true;
static bool car_wheel_down = false;
...
int azure_pnp_handle_command_request(azure_iot_t* azure_iot, command_request_t command)
{
  ...
  if (az_span_is_content_equal(command.command_name, COMMAND_NAME_FORWARD))
  {
    digitalWrite(IN1, car_wheel_up);
    digitalWrite(IN2, car_wheel_down);
    digitalWrite(IN3, car_wheel_down);
    digitalWrite(IN4, car_wheel_up);
    LogInfo("LED 1 state: %s", (car_wheel_up ? "ON" : "OFF"));
    response_code = COMMAND_RESPONSE_CODE_ACCEPTED;
  }
  else if (az_span_is_content_equal(command.command_name, COMMAND_NAME_BACKWARD))
  {

    digitalWrite(IN1, car_wheel_down);
    digitalWrite(IN2, car_wheel_up);
    digitalWrite(IN3, car_wheel_up);
    digitalWrite(IN4, car_wheel_down);
    LogInfo("LED 2 state: %s", (car_wheel_down ? "ON" : "OFF"));
    response_code = COMMAND_RESPONSE_CODE_ACCEPTED;
  }
  else if (az_span_is_content_equal(command.command_name, COMMAND_NAME_LEFT))
  {
    digitalWrite(IN1, car_wheel_up);
    digitalWrite(IN2, car_wheel_down);
    digitalWrite(IN3, car_wheel_up);
  ...
  else if (az_span_is_content_equal(command.command_name, COMMAND_NAME_RIGHT))
  {
  ...
  else if (az_span_is_content_equal(command.command_name, COMMAND_NAME_STOP))
  {
